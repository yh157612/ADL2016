...
class GloVeModel():
    ...
    def fit_to_corpus(self, corpus):
        self.__my_fit_to_corpus(corpus, self.max_vocab_size, self.min_occurrences,
    ...
    def __my_fit_to_corpus(self, corpus, vocab_size, min_occurrences, left_size,
                           right_size):
        self.__words = [word for word, count
                        in Counter(corpus).most_common(vocab_size)
                        if count >= min_occurrences]
        self.__word_to_id = {word: i for i, word in enumerate(self.__words)}
        data = [self.__word_to_id.get(word, -1) for word in corpus]
        cooccurrence_matrix = defaultdict(float)
        for l_context, word, r_context in _my_context_windows(data, left_size, right_size):
            # print(l_context, word, r_context, sep='\n', end='\n\n')
            if word < 0:
                continue
            for i, context_word in enumerate(reversed(l_context)):
                if context_word >= 0:
                    cooccurrence_matrix[word, context_word] += 1 / (i + 1)
            for i, context_word in enumerate(r_context):
                if context_word >= 0:
                    cooccurrence_matrix[word, context_word] += 1 / (i + 1)
    ...
    def train(self, num_epochs, log_dir=None, summary_batch_interval=None,
              tsne_epoch_interval=None, save_embed_interval=None):
        ...
        should_save_embeddings = log_dir is not None and save_embed_interval
        ...
        with tf.Session(graph=self.__graph) as session:
            ...
            for epoch in range(num_epochs):
                ...
                if should_save_embeddings and (epoch + 1) % save_embed_interval == 0:
                    current_embeddings = self.__combined_embeddings.eval()
                    output_path = os.path.join(log_dir, "embed{:03d}.txt".format(epoch + 1))
                    self.save_embeddings(output_path, current_embeddings)
            self.__embeddings = self.__combined_embeddings.eval()
            if should_write_summaries:
                summary_writer.close()

    def _train_thread_body(self, session, batches):
        for batch_index, batch in enumerate(batches):
            i_s, j_s, counts = batch
            if len(counts) != self.batch_size:
                continue
    ...
    def train_concurrent(self, num_epochs, num_threads=12, log_dir=None, save_embed_interval=None):
        should_save_embeddings = log_dir is not None and save_embed_interval
        batches = self.__prepare_batches()
        batches_list = []
        num_batches = len(batches) // num_threads
        for i in range(num_threads - 1):
            batches_list.append(batches[i * num_batches:(i + 1) * num_batches])
        batches_list.append(batches[(num_threads - 1) * num_batches:])
        with tf.Session(graph=self.__graph) as session:
            tf.initialize_all_variables().run()
            for epoch in range(num_epochs):
                # shuffle(batches)
    ...
    def save_embeddings(self, output_path, embeddings=None):
        if embeddings is None:
            embeddings = self.__embeddings
        os.makedirs(os.path.dirname(output_path), mode=0o755, exist_ok=True)
        with open(output_path, 'w') as f:
            for word, id in self.__word_to_id.items():
                f.write(word)
                for j in embeddings[id]:
                    f.write(' ')
                    f.write(str(j))
                f.write('\n')

    def __prepare_batches(self, cooccurrence_matrix=None):
...
def _my_context_windows(region, left_size, right_size):
    left_context = deque(maxlen=left_size)
    right_context = deque(region[:right_size], maxlen=right_size)
    for i, word in enumerate(region):
        right_context.popleft()
        if i + right_size < len(region):
            right_context.append(region[i + right_size])
        yield left_context, word, right_context
        left_context.append(word)
