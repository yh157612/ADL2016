...
class Word2Vec(object):
  ...
  def read_analogies(self):
    ...
    """
    if not self._options.eval_data:
      return
    questions = []
    questions_skipped = 0
  ...
  def save_embeddings(self, file_path):
    ...
    with open(file_path, "w") as f:
      for word, id in self._word2id.items():
        f.write(word.decode())
...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    # model.save_embeddings(os.path.join(opts.save_path, "word2vec_embed.txt"))
    model.save_embeddings(opts.save_path)
