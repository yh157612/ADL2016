...
parser.add_argument("--eval-epoch-steps", type=int, default=25000, help="how many steps (=4 frames) to run during an eval epoch (approx -- will finish current game)")
parser.add_argument("--replay-capacity", type=int, default=2000000, help="how many states to store for future training")
...
parser.add_argument("--screen-capture-freq", type=int, default=2500000, help="record screens for a game this often")
...
def runEpoch(minEpochSteps, evalWithEpsilon=None):
    ...
    while environment.getStepNumber() - stepStart < minEpochSteps:
        ...
        sys.stdout.write('\r%3d%% %s %d ended with score: %d (%d frames in %fs for %d fps)      ' %
            (int(100 * (environment.getStepNumber() - stepStart) / minEpochSteps),
...
while True:
    ...
    print('Average training score: %f' % (aveScore))
    aveScore = runEpoch(args.eval_epoch_steps, evalWithEpsilon=.05) #eval
    print('Average eval score: %f' % (aveScore))
